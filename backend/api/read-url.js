...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!url || !/^https?:\/\//i.test(url)) {
      return json(res, 400, { ok: false, error: 'Missing valid URL. Use /api/read-url?url=https://example.com or POST {"url":"https://example.com"}.' });
    }

    const apiKey = getEnv('FIRECRAWL_API_KEY');
    if (!apiKey) {
      return json(res, 500, { ok: false, error: 'FIRECRAWL_API_KEY is not configured on backend.' });
    }
