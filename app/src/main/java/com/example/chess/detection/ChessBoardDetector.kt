...
/**
 * Dependency-free detector for regular two-colour 8x8 chess boards.
 * It searches square regions and scores how consistently their sampled corner
 * colours alternate. Piece occupancy is estimated from the difference between
 * the square background and its centre region.
 */
class ChessBoardDetector {
    ...
    private fun scoreCandidate(bitmap: Bitmap, left: Int, top: Int, size: Int): Candidate {
        ...
        val firstVariance = colourSpread(lightSamples, first)
        val secondVariance = colourSpread(darkSamples, second)
        val variancePenalty = (firstVariance + secondVariance) * 0.55
        val score = separation - variancePenalty
        return Candidate(left, top, size, score)
    }
