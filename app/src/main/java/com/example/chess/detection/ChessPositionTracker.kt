...
class ChessPositionTracker {
    ...
    fun update(detection: BoardDetectionResult): PositionTrackingResult {
        ...
        if (board == null) {
            ...
            lastFen = fen
            return PositionTrackingResult.Position(fen, true, orientation)
        }
        ...
        if (sameOccupancy(oldOccupied, occupied) &&
            ...
            return PositionTrackingResult.Position(
                FenConverter.toFen(board!!, whiteToMove),
                false,
                orientation
            )
        }
        ...
        lastFen = fen
        return PositionTrackingResult.Position(fen, changed, orientation)
    }
    ...
    private fun inferMove(
        ...
        for (row in 0 until 8) {
            for (col in 0 until 8) {
                ...
                if (piece?.color == currentColor && oldOccupied[row][col] && !newOccupied[row][col]) {
                    sources += row * 8 + col
                }
            }
        }
        ...
        for (row in 0 until 8) {
            for (col in 0 until 8) {
                ...
                if (becameOccupied || visualChange >= MIN_SIGNATURE_CHANGE) {
                    candidates += destination to visualChange
                }
            }
        }

        return candidates
            .filter { (destination, _) -> isPseudoLegal(currentBoard, source, destination, piece) }
    ...
    private fun inferCastling(
        ...
        val kingSource = sources.firstOrNull {
            currentBoard[it / 8][it % 8]?.type == ChessPieceType.KING
        } ?: return null
        val rookSource = sources.firstOrNull {
            currentBoard[it / 8][it % 8]?.type == ChessPieceType.ROOK
        } ?: return null

        val row = kingSource / 8
        val kingCol = kingSource % 8
        val rookCol = rookSource % 8
        if (row != rookSource / 8 || kingCol != 4 || rookCol !in intArrayOf(0, 7)) return null
        val destinationCol = if (rookCol == 7) 6 else 2
        return if (newOccupied[row][destinationCol]) {
            kingSource to (row * 8 + destinationCol)
        } else null
    }
    ...
    private fun initialBoard(): Array<Array<ChessPiece?>> {
        val result: Array<Array<ChessPiece?>> = Array(8) { arrayOfNulls(8) }
