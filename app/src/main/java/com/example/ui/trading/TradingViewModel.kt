...
class TradingViewModel(
    ...
    fun fetchBtcNews() {
        fetchData("news", "$backendUrl/api/search?q=btc%20bitcoin%20news") { state, data ->
            state.copy(newsData = data, isLoading = false, error = null)
        }
    }
    ...
    private fun executeDemoOrder(action: String, symbol: String, quantity: Double) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                _uiState.update { it.copy(isLoading = false, error = "Network Error: ${e.javaClass.simpleName} - ${e.message ?: "No detail"}") }
            }
        }
    }
